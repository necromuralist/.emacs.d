...
* Things To Do [0/6]
- [ ] Move setup so far to separate posts and then include elisp back into this one
...
* Includes
The =#+INCLUDE= doesn't get picked up when you tangle, only when you export this file to another format. To make this work:

- Export this file to another Org file (C-c C-e Ov)
  This will export it to a file named =the-init-file.org.org= and open it in another buffer
- Change into the new buffer and tangle the configuration there (C-c C-v C-t)
